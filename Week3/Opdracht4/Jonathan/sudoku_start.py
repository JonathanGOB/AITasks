...
def prepare_data(grid):
    full_data = np.array()
    list_alpha = list(string.ascii_uppercase)
    for key, value in grid.items():
        row_data = np.array()
        for number in value:
            row_insert = "{0:b}".format(list_alpha.index(key[0])) + "{0:b}".format(int(key[1]))\
                       + "{0:b}".format(str(number)) + "{0:b}".format(list_alpha.index(key[0]) * int(key[1])) + \
                       "{0:b}".format(list_alpha.index(key[0]) * int(number)) + \
                       "{0:b}".format(list_alpha.index(key[1]) * int(number)) + [i
                        for i, box in enumerate([cross(rs, cs) for rs in ('ABC','DEF','GHI')
                                                 for cs in ('123','456','789')]) if key in box][0] * int(number)
...
def solve(grid):
    ...
    for key, value in grid.items():
        ...
        for number in value:
            row_insert = "{0:b}".format(list_alpha.index(key[0])) + "{0:b}".format(int(key[1])) \
                         + "{0:b}".format(int(number)) + "{0:b}".format(list_alpha.index(key[0]) * 9 + int(key[1])) + \
                         "{0:b}".format(list_alpha.index(key[0]) + int(number)) + \
                         "{0:b}".format(int(key[1]) + int(number)) + "{0:b}".format([i for i, box in enumerate(
                    [cross(rs, cs) for rs in ('ABC', 'DEF', 'GHI')
                     for cs in ('123', '456', '789')]) if key in box][0] + int(number))
            print(len(row_insert))
