    ...
    if start_position:  # search next from this state
        for e in neighborOffsets:
            ...
            if board.playground[e[0] + temp[0]][e[1] + temp[1]][1] == 0:
                if word + board.playground[e[0] + temp[0]][e[1] + temp[1]][0] in prefixes or word + \
                        board.playground[e[0] + temp[0]][e[1] + temp[1]][0] in full_words:
                    new_board = board.add_child(copy.deepcopy(board))
                    new_board.playground[e[0] + temp[0]][e[1] + temp[1]][1] = (start_position[0] + 1) * (
                                start_position[1] + 1)
...
if __name__ == "__main__":
    board = Board([[[random.choice(string.ascii_lowercase), 0] for e in range(4)] for e in range(4)])
    print(board)
